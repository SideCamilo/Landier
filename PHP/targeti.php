	...
	<div class = "container">
				<div class = "row">
					...
					<div class = "col-md-4" style = "height: 500px; overflow: scroll; color: white; border:solid  #ffffff; border-width: 2px;">';
						if($row_sql_buscar['codigo'] != ""){
							$output_string .= '<h1> <b>Codigo:</b>'.$row_sql_buscar['codigo'].'</h1>';
						}
						if($row_sql_buscar['valor_venta'] != ""){
							$output_string .= '<h1> <b>Valor:</b>'.$row_sql_buscar['valor_venta'].'</h1>';
						}
						if($row_sql_buscar['terreno'] != ""){
							$output_string .= '<h1> <b>Terreno:</b>'.$row_sql_buscar['terreno'].'</h1>';
						}
						if($row_sql_buscar['construccion'] != ""){
							$output_string .= '<h1> <b>Construcción:</b>'.$row_sql_buscar['construccion'].'</h1>';
						}
						if($row_sql_buscar['banos'] != ""){
							$output_string .= '<h1> <b>Baños:</b>'.$row_sql_buscar['banos'].'</h1>';
						}
						if($row_sql_buscar['medbanos'] != ""){
							$output_string .= '<h1> <b>Medios Baños:</b>'.$row_sql_buscar['medbanos'].'</h1>';
						}
						if($row_sql_buscar['plantas'] != ""){
							$output_string .= '<h1> <b>Plantas:</b>'.$row_sql_buscar['plantas'].'</h1>';
						}
						if($row_sql_buscar['recamaras'] != ""){
							$output_string .= '<h1> <b>Recámaras:</b>'.$row_sql_buscar['recamaras'].'</h1>';
						}
						if($row_sql_buscar['nombre_zona'] != ""){
							$output_string .= '<h1> <b>Zona:</b>'.$row_sql_buscar['nombre_zona'].'</h1>';
						}
						if($row_sql_buscar['ciudad'] != ""){
							$output_string .= '<h1> <b>Ciudad:</b>'.$row_sql_buscar['ciudad'].'</h1>';
						}
						if($row_sql_buscar['colonia'] != ""){
							$output_string .= '<h1> <b>Colonia:</b>'.$row_sql_buscar['colonia'].'</h1>';
						}
						if($row_sql_buscar['nombre_estado'] != ""){
							$output_string .= '<h1> <b>Estado:</b>'.$row_sql_buscar['nombre_estado'].'</h1>';
						}
						if($row_sql_buscar['telefono'] != ""){
							$output_string .= '<h1> <b>Teléfono:</b>'.$row_sql_buscar['telefono'].'</h1>';
						}
						if($row_sql_buscar['descripcion'] != ""){
							$output_string .= '<h1> <b>Descripción:</b>'.$row_sql_buscar['descripcion'].'</h1>';
						}
						if($row_sql_buscar['inversion'] != ""){
							$output_string .= '<h1> <b>Inversión:</b>'.$row_sql_buscar['inversion'].'</h1>';
						}
						if($row_sql_buscar['incluyeinversion'] != ""){
							$output_string .= '<h1> <b>Incluye Inversión:</b>'.$row_sql_buscar['incluyeinversion'].'</h1>';
						}
						if($row_sql_buscar['proyecto'] != ""){
							$output_string .= '<h1> <b>Proyecto:</b>'.$row_sql_buscar['proyecto'].'</h1>';
						}
						if($row_sql_buscar['rendimiento'] != ""){
							$output_string .= '<h1> <b>Rendimiento:</b>'.$row_sql_buscar['rendimiento'].'</h1>';
						}
						if($row_sql_buscar['cadena'] != ""){
							$output_string .= '<h1> <b>Cadena:</b>'.$row_sql_buscar['cadena'].'</h1>';
						}
						if($row_sql_buscar['tiempo'] != ""){
							$output_string .= '<h1> <b>Tiempo:</b>'.$row_sql_buscar['tiempo'].'</h1>';
						}
						$output_string .= '
					</div>
				</div>
